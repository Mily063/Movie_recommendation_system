...
class CollaborativeFilteringModel(RecommendationModel):
    ...
    def _create_dummy_ratings(self):
        ...
        for _, row in self.movies_df.iterrows():
            ...
            year = int(row['release_date'][:4]) if 'release_date' in row and str(row['release_date'])[:4].isdigit() else 2000
            vote = float(row.get('vote_average', 3))

            # user 1 lubi konkretny gatunek, np. 'Action'
            ratings.append({'user_id': 1, 'movie_id': gid, 'rating': 5.0 if 'Action' in str(genres) else 2.0})

            # user 2 lubi nowe i dobrze oceniane
            ratings.append({'user_id': 2, 'movie_id': gid, 'rating': 5.0 if vote > 7 and year > 2015 else 2.0})
    ...
    def recommend(self, preferences: dict, n: int = 5):
        ...
        candidates_df = candidates_df.sort_values(['collab_score', 'id'], ascending=[False, True])
